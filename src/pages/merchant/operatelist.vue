        ...
        <div class="right_body">
                ...
                <el-row :gutter="10">
                    <el-col :xs="24" :sm="24" :md="24" :lg="24">
                        <el-form-item :label="$t('merchantlList.searchBank.operateTime')+':'" prop="">
                            <el-date-picker v-model="searchkey.s_op_dtm" type="datetime" align="right" placeholder="" :picker-options="pickerOptions_s" format="yyyy-MM-dd HH:mm:ss" :default-time="['00:00:00']" popper-class="no_now" @change="start_change" :editable="false" :clearable="false">
                            </el-date-picker>
                            <span>至</span>  
                            <el-date-picker v-model="searchkey.e_op_dtm" type="datetime" align="right" placeholder="" :picker-options="pickerOptions_e" format="yyyy-MM-dd HH:mm:ss" :default-time="['23:59:59']" popper-class="no_now noAnotherMonth" :editable="false" :clearable="false">
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
                    ...
                    <el-col :xs="24" :sm="24" :md="12" :lg="6">
                        <el-form-item :label="$t('merchantlList.searchBank.operateStyle')+':'">
                            <el-select v-model="searchkey.optype" placeholder="">
                                <el-option :label="$t('app.all')" value=""></el-option>
                                <el-option :label="$t('merchantlList.searchBank.createMuch')" value="1"></el-option>
                                <el-option :label="$t('merchantlList.searchBank.mendMuch')" value="2"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :xs="24" :sm="24" :md="12" :lg="6">
            ...
            <template>
                <el-table :data="transfer_list_now" border stripe style="width: 100%">
                    <el-table-column prop="optime" :label="$t('merchantlList.searchBank.operateTime')" resizable min-width="180px">
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('merchantlList.bodyBank.detail')" resizable min-width="100px">
                        <template scope="scope">
                            <el-button type="text" @click="getDetail(scope.row.opid)" v-if="scope.row.status==2">{{$t('merchantlList.bodyBank.detail')}}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </template>
...
export default {
  ...
  data() {
    return {
      ...
      list_url: "/qudao/v1/api/chain/oprecord/list", //获取列表
      down_url: "/qudao/v1/api/chain/oprecord/download_detail",
      pages_all: 0, //总信息数
      pages: 1, //总页数
      page_per: 20, //每页信息数
      page_now: 1, //当前页数
      ...
      pickerOptions_e: {
        disabledDate: time =>
          time.getTime() > this.last_day || time.getTime() < this.start_day
      },
      searchkey_rule: {}
    };
  },
  created: function() {
    ...
    this.searchkey.s_op_dtm = this.get_daystart();
    this.searchkey.e_op_dtm = new Date();
    setTimeout(() => {
  ...
  methods: {
    ...
    get_list: function() {
      let _this = this;
      if (this.searchkey.s_op_dtm - this.searchkey.e_op_dtm > 0) {
        this.$message({
      ...
      let post_data = {
        ...
        optype: _this.searchkey.optype,
        s_op_dtm: _this.get_datetime(_this.searchkey.s_op_dtm),
        e_op_dtm: _this.get_datetime(_this.searchkey.e_op_dtm)
      };
      this.$ajax_log.ajax_get(this, this.list_url, post_data, data_return => {
        _this.pages_all = data_return.data.total_cnt;
        _this.transfer_list_now = data_return.data.op_infos;
      });
    },
    //提交查询--后台处理
    ...
    start_change(time) {
      ...
      //结束时间--年月日
      let end_t = this.searchkey.e_op_dtm;
      ...
      if (year !== _this.now_year || month !== _this.now_month) {
        ...
        _this.searchkey.e_op_dtm = lastday;
      } else {
        _this.last_day = new Date();
        _this.searchkey.e_op_dtm = new Date();
      }
    },
    //获取当月1号0点的时间
