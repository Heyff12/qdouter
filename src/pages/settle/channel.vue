      ...
      <el-form label-width="100px" class="demo-ruleForm" :model="searchkey" :rules="searchkey_rule" ref="searchkey">
        <el-row :gutter="10">
          <el-col :xs="24" :sm="24" :md="12" :lg="5">
            <el-form-item label="渠道编号：" prop="mchnt_userid">
              <el-input v-model="searchkey.mchnt_userid"></el-input>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="5">
            <el-form-item label="渠道名称：" prop="mchnt_name">
...
export default {
  ...
  data() {
    return {
      ...
      list_url: location.protocol + '//' + location.host + '/qudao/v1/api/settlement/qd_royalty/list', //获取交易列表      
      down_url: location.protocol + '//' + location.host + '/qudao/v1/api/settlement/qd_royalty/list/download', //下载                  
      pages_all: 0, //总信息数
      pages: 1, //总页数
      page_per: 10, //每页信息数
      page_now: 1, //当前页数
      ...
      searchkey_rule: {
        ...
        mchnt_name: yanzheng.test_qd_name(this.$t('generalPro.bodyBank.qdName'), 1, 30, false, 'blur', this),
      },
    }
  },
  created: function() {
  ...
  methods: {
    ...
    get_list: function() {
      ...
      let post_data = {
        ...
        'qd_uid': _this.searchkey.mchnt_userid.replace(/\s/g, ''),
        'qd_name': _this.searchkey.mchnt_name.replace(/\s/g, ''),
