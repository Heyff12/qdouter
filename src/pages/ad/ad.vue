    ...
    <div>
        ...
        <bounced :visible="add_ad" :newclass="big_bounced">
            ...
            <el-form :model="add_ader" :rules="add_ader_rules" ref="add_ader" label-width="110px" class="demo-ruleForm">
                ...
                <el-form-item label="">
                    <template v-if="if_right_class==true && if_clickSure==true && ifImg==true">
...
export default {
    ...
    data() {
        return {
            ...
            list_url: '/qudao/adv/banner/list', //获取交易列表 
            adder_url: '/qudao/adv/banner/create', //增加广告主   
            adder_fix_url: '/qudao/adv/banner/edit', //修改广告主   
            adder_to_url: '/qudao/adv/redirect/create', //广告跳转链接创建接口                                
            pages_all: 0, //总信息数
            pages: 1, //总页数
            page_per: 10, //每页信息数
            page_now: 1, //当前页数
    ...
    methods: {
        ...
        add_ader_sub: function() {
            ...
            let _this = this; 
            if(this.add_ader.display_page==1 && !this.testUrl(this.add_ader.image_url)){
                this.visible_toast = true;
                this.toastmsg = '请输入图片链接!';
                this.ifImg =false;
                return false;
            }
            this.ifImg =true;
            this.$refs.add_ader.validate((valid) => {
                if (valid) {
                    //_this.to_sub();    
                    _this.adder_sub();
                    // console.log(this.testUrl(this.add_ader.image_url))       
                    // if(this.add_ader.display_page==1 && !this.testUrl(this.add_ader.image_url)){
                    //     this.visible_toast = true;
                    //     this.toastmsg = '请输入图片链接!';
                    // }else{
        ...
        adder_sub: function() {
            ...
            if(this.add_ader.display_page==1){
                post_data.image_url=this.add_ader.image_url;
            }
            if (this.add_or_fix) {
