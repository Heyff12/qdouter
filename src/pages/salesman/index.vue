<template>
    <div>
        <div class="right_head">
            {{$t('salesmanlList.titleBank.title')}}
        </div>
        <div class="right_body">
            <el-form label-width="100px" class="demo-ruleForm" :model="searchkey" :rules="searchkey_rule" ref="searchkey">
                <el-row :gutter="10">
                    <el-col :xs="24" :sm="24" :md="12" :lg="5">
                        <el-form-item :label="$t('generalPro.searchBank.salesmanNum')" prop="slsm_uid">
                    ...
                    <el-col :xs="24" :sm="24" :md="12" :lg="5">
                        <el-form-item :label="$t('generalPro.searchBank.tel')" prop="slsm_mobile">
                            <el-input v-model.trim="searchkey.slsm_mobile"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :xs="24" :sm="24" :md="12" :lg="5">
                        <el-form-item :label="$t('generalPro.searchBank.salesmanStatus')">
                            <el-select v-model="searchkey.slsm_status">
                                <el-option :label="$t('app.all')" value=""></el-option>
                                <el-option :label="$t('generalPro.searchBank.onJob')" value="0"></el-option>
                                <el-option :label="$t('generalPro.searchBank.dimission')" value="1"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
            ...
            <template>
                <el-table :data="salesmen_now" border stripe style="width: 100%">
                    <el-table-column prop="slsm_uid" :label="$t('generalPro.bodyBank.salesmanNum')" resizable min-width="80px">
                    </el-table-column>
                    <el-table-column prop="slsm_name" :label="$t('generalPro.bodyBank.salesmanName')" resizable min-width="120px">
                    </el-table-column>
                    <el-table-column prop="slsm_mobile" :label="$t('generalPro.bodyBank.tel')" resizable min-width="100px">
                    </el-table-column>
                    <el-table-column prop="slsm_status" :label="$t('generalPro.bodyBank.salesmanStatus')" resizable>
                        <template scope="scope">
                            <span v-if="scope.row.slsm_status">{{$t('generalPro.searchBank.dimission')}}</span>
                            <span v-else>{{$t('generalPro.searchBank.onJob')}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column :label="$t('app.operate')" resizable min-width="240px">
                        <template scope="scope">
                            ...
                            <template v-if="!scope.row.slsm_status">
                                <el-button type="warning" @click="open_dialog_cancle(scope.row.slsm_uid)">{{$t('generalPro.bodyBank.deactive')}}</el-button>
                            </template>
                        </template>
                    </el-table-column>
                </el-table>
        ...
        <bounced :visible="dialogslsm" :newclass="big_bounced">
            <span slot="header">{{$t('generalPro.searchBank.salesmanName')}}{{saleman.slsm_name}}<i class="close"  @click="dialogslsm = false"></i></span>
            <el-table :data="saleman.monthly">
                <el-table-column align="center" property="date" :label="$t('salesmanlList.bodyBank.month')" resizable min-width="100px"></el-table-column>
                <el-table-column align="center" property="mchnt_count" :label="$t('salesmanlList.bodyBank.merchantNum')" resizable min-width="100px"></el-table-column>
        ...
        <bounced :visible="dialogcancle" :newclass="big_bounced">
            <span slot="header">{{$t('salesmanlList.bodyBank.inactive')}}<!-- <i class="close"  @click="dialogcancle = false"></i> --></span>
            <input type="hidden" name="" v-model="cancle_id" />
            <el-form :model="sale" :rules="rules" ref="sale" label-width="100px" class="demo-ruleForm">
            ...
            <p slot="footer">
                <span class="bounced_button bounced_sure" @click="mendcancle">{{$t('app.sure')}}</span>
                <span class="bounced_button bounced_cancle"  @click="dialogcancle = false">{{$t('app.cancle')}}</span>
            </p>
        </bounced>
    </div>
</template>
...
export default {
        ...
        return {
            ...
            rules: {
                sale_sel: [{
                    type: "number",
                    required: true,
                    message: this.$t('yanzheng.salesmanSel'),
                    trigger: 'blur'
                }],
            },
            ...
            searchkey_rule: {
                slsm_uid: [{
                    required: false,
                    pattern: /^[0-9\s]{1,200}$/,
                    message: this.$t('yanzheng.salesmanNum'),
                    trigger: 'blur'
                }],
                slsm_name: [{
                    required: false,
                    pattern: /^[\u4e00-\u9fa5a-zA-Z0-9\s]{1,30}$/,
                    message: this.$t('yanzheng.salesmanName'),
                    trigger: 'blur'
                }],
                slsm_mobile: [{
                    required: false,
    ...
    methods: {
        ...
        get_saleses: function() {
            let _this = this;
            this.$ajax_log.ajax_get(this, this.slsmes_url + _this.cancle_id + '/disable/transfer_list', '', (data_return) => {
        ...
        mendcancle_post() {
            ...
            this.$ajax_log.ajax_post(this, this.slsmes_cancle_url + _this.cancle_id + '/disable', post_data, (data_return) => {
        ...
        open_dialog(id) {
            let _this = this;
            let slsm_id = id;
            this.$ajax_log.ajax_get(this, this.slsm_stat_url + slsm_id + '/stat', '', (data_return) => {
