        ...
        <header>
          <div class="nav_menu_logo">
            <router-link :to="{ name: 'home' }">
              ...
              <br/>
              <!-- {{base.short_name}} -->{{$store.state.qudao_name}}
            </router-link>
          </div>
          <el-menu default-active="1" class="el-menu-vertical-demo" :unique-opened="true" :router="true" @open="handleOpen" @close="handleClose">
            ...
            <el-submenu index="11" v-show="searchkey.lan=='zh'">
              <template slot="title"><i class="icon_left icon_nocard_w"></i>{{$t('menus.noCard')}}</template>
              <el-menu-item index="/no_merchant">{{$t('menus.noMerchant')}}</el-menu-item>
            </el-submenu>
            <el-submenu index="13" v-show="searchkey.lan=='zh'">
              <template slot="title"><i class="icon_left icon_busy_w"></i>{{$t('menus.busyCircle')}}</template>
            ...
            <!-- <el-submenu index="14" v-if="$store.state.if_alliance==2  && searchkey.lan=='zh'">
              ...
              <el-menu-item index=""><a href="https://qudao.qfpay.com/bizAlliances/" target="_blank">{{$t('menus.busyAlliance')}}</a></el-menu-item>
            </el-submenu> -->
          </el-menu>
          ...
          <el-row :gutter="0">
            ...
            <el-col :xs="24" :sm="24" :md="24" :lg="24" :offset="0">
              <div class="manage_head_r">
                ...
                <dt>
                  <!-- {{base.name}} -->{{$store.state.qudao_name}}</dt>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      if_alliance_url: '/qudao/v1/api/entrance/status', //获取特定外部模块入口状态,判断商圈联盟 栏目 是否显示
      read_detail: {
        "announce_id": '', // 公告 id
        "title": "",
        "content": "",
      ...
      // allianceUrl: location.host.indexOf('qudao.qa.qfpay.net') > -1 ? '/bizAlliances/main.html' : '/bizAlliances/',
      allianceUrl: '/bizAlliances/',
    }
  },
  created: function() {
  ...
  computed: {
    login: function() {
      ...
      if (!if_login && !name) { //不在登录页，用户名不存在
        ...
        this.get_notice(); //获取通知弹框
        this.get_ifAd(); //获取是否开通广告管理 
        this.get_ifAlliance(); //获取是否开通商圈联盟 
  ...
  methods: {
    ...
    //判断商圈联盟 栏目 是否显示
    get_ifAlliance: function() {
      var _this = this;
      this.$ajax_log.ajax_get(this, this.if_alliance_url, { 'module_name': 'alliance' }, (data_return) => {
        _this.$store.commit('t_if_alliance', data_return.data.status); //该外部模块入口状态， 1 关闭 2 开启
      });
    },
    //获取通知弹框
    get_notice: function() {
      ...
      .manage_head_r {
        ...
        span {
          ...
          dl {
            ...
            right: 0;
            display: none;
            background: @orange; // width: 100%+20;
            min-width: 220px;
            z-index: 100;
            border-radius: 3px;
...
.header {
  .el-menu {
    ...
    .el-menu-item {
      ...
      i.icon_left {
        ...
        &.icon_alliance_w {
          background: url('./img/ic_alliance_iw.png') center center no-repeat;
      ...
      &.is-active {
        ...
        i.icon_left {
          ...
          &.icon_alliance_w {
            background: url('./img/ic_alliance.png') center center no-repeat;
            background-size: 18px auto;
            top: 2px;
          }
        }
  ...
  .el-submenu {
    ...
    i.icon_left {
      ...
      &.icon_nocard_w {
        background: url('./img/ic_nocard_iw.png') center center no-repeat;
        background-size: 16px auto;
        top: 2px;
      }
      &.icon_busy_w {
        background: url('./img/ic_busy_iw.png') center center no-repeat;
        background-size: 16px auto;
        top: 2px;
      }
      &.icon_alliance_w {
    ...
    &.is-opened {
      i.icon_left {
        ...
        &.icon_nocard_w {
          background: url('./img/ic_nocard.png') center center no-repeat;
          background-size: 16px auto;
          top: 2px;
        }
        &.icon_busy_w {
...
//自定义弹框样式bounced------------------------z-index:  1010 1011;
.bounced_my {
  width: 100%;
  height: 100%;
  position: fixed;
  ...
  .bounced_my_body {
    ...
    &.big_bounced {
      ...
      max-height: 100%; // margin-left: -230px;
      // top: 30%;
      // width: 60%; 
      // height: 80%;
      header {
